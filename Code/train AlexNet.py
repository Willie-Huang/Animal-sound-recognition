...
from keras.layers import (Conv2D, MaxPooling2D, GlobalAveragePooling2D,
                          ...
                          Reshape, GlobalAveragePooling1D, Dense, Dropout,
...
AUGMENT_WIND = True         # Only for TRAIN; no augmentation for TEST
AUG_PER_CLEAN = 4           # Generate 4 noisy versions for each clean sample
SNR_CANDIDATES = [15, 10, 5, 20, 0]
SNR_WEIGHTS    = [3,  4,  3,  1,  0.3]   # Make 10–15 dB more common and 0 dB rare
PREEMPH_ALPHA  = 0.95       # Mild pre-emphasis to suppress low-frequency wind noise
SEED = 42
...
# ---------- Model choice ----------
MODEL_TYPE = "alexnet"   # options: "alexnet", "ast_tiny"

def build_alexnet_mels(input_shape, num_classes):
    """
    AlexNet-like 2D CNN for log-Mel spectrograms.
    Input: (T, F) -> reshape to (T, F, 1)
    """
    inputs = Input(shape=input_shape)  # (T, F)
    x = Reshape((input_shape[0], input_shape[1], 1))(inputs)

    # Conv1: large kernel + stride to quickly compress time/frequency resolution
    x = Conv2D(64, (11, 11), strides=(2, 2), activation='relu', padding='same')(x)
    ...
    return tf.keras.Model(inputs, outputs, name="AlexNet_Mels")

def build_ast_tiny(input_shape, num_classes, patch_time=16, patch_freq=16, d_model=256, num_heads=4, mlp_dim=512, depth=4):
    """
    Minimal AST-like Transformer for log-Mel.
    Patchify (T,F) -> tokens -> Transformer encoder x depth -> GAP -> softmax
    """
    T, F = input_shape
    inputs = Input(shape=(T, F))  # (time, mel)

    # If T/F are not multiples of the patch size, pad on the right to the nearest multiples
    pad_T = (patch_time - (T % patch_time)) % patch_time
    pad_F = (patch_freq - (F % patch_freq)) % patch_freq
    x = tf.pad(inputs, [[0, 0], [0, pad_T], [0, pad_F]])

    # (T',F',1) for extract_patches
    x = tf.expand_dims(x, axis=-1)
    patches = tf.image.extract_patches(
        images=x,
        sizes=[1, patch_time, patch_freq, 1],
        strides=[1, patch_time, patch_freq, 1],
        rates=[1, 1, 1, 1],
        padding='VALID'
    )
    # Flatten each patch to a vector
    patch_dim = patch_time * patch_freq  # single channel
    tokens = Reshape((-1, patch_dim))(patches)      # (B, N, P)
    # Linear projection to d_model
    tokens = Dense(d_model)(tokens)

    # Learnable positional encoding (dynamic-length safe)
    num_tokens = tf.shape(tokens)[1]
    positions = tf.range(num_tokens)[tf.newaxis, :]  # (1, N)
    pos_embed_layer = Embedding(input_dim=1000, output_dim=d_model)  # allow up to 1000 patches; increase if needed
    pos_emb = pos_embed_layer(positions)  # (1, N, d_model)
    x = tokens + pos_emb
    ...
    for _ in range(depth):
        ...
        # FFN + residual + LN
        ffn = Dense(mlp_dim, activation='relu')(x)
        ffn = Dense(d_model)(ffn)
        x = LayerNormalization(epsilon=1e-6)(x + ffn)

    # Global pooling to get utterance-level representation (average over tokens)
    x = GlobalAveragePooling1D()(x)
    x = Dropout(0.4)(x)
    outputs = Dense(num_classes, activation='softmax')(x)
    return tf.keras.Model(inputs, outputs, name="AST_tiny")

# ---------- Build & compile ----------
input_shape = (MAX_LENGTH, N_MELS)

if MODEL_TYPE.lower() == "alexnet":
    model = build_alexnet_mels(input_shape, num_classes)
elif MODEL_TYPE.lower() == "ast_tiny":
    model = build_ast_tiny(input_shape, num_classes)
else:
...
# ---------- Save ----------
model_path = os.path.join(MODEL_DIR, "animal_sound_model.keras")  # Keras 3 native format
